...
import { shouldShowCountdown, getCountdownBlurClass } from '@/lib/countdown-config';

const Index = () => {
  const [snowEnabled, setSnowEnabled] = useState(true);
  const [showCountdown, setShowCountdown] = useState(shouldShowCountdown);
  const [showWelcome, setShowWelcome] = useState(() => {
    if (showCountdown) return false; // No mostrar welcome si hay countdown
  ...
  const handleCountdownComplete = () => {
    ...
    // Después del countdown, mostrar welcome si nunca se ha visitado
    const hasVisited = localStorage.getItem('has-visited');
    if (hasVisited !== 'true') {
      setShowWelcome(true);
    }
  ...
  return (
    <div className="min-h-screen w-full flex flex-col">
      ...
      <div className={`flex flex-col min-h-screen transition-all duration-1000 ${getCountdownBlurClass()}`}>
        <Header />
